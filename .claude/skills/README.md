...
|----------|--------|----------------|
| **Cursor** | MDC Rules | Run `.\setup-cursor-rules.ps1` or `./setup-cursor-rules.sh` |
| **Claude Code** | Plugin | `/plugin install czlonkowski/n8n-skills` |
| **Claude.ai** | Upload | Settings → Capabilities → Skills → Upload zip |
...
### For Cursor Users (MDC Rules)

Convert skills to Cursor's MDC rule format for automatic activation:

**PowerShell (Windows):**
```powershell
# Basic setup (7 core skill files)
.\setup-cursor-rules.ps1 -TargetProject "C:\path\to\your\n8n-project"

# Include all reference files (detailed docs)
.\setup-cursor-rules.ps1 -TargetProject "." -IncludeReferences
```

**Bash (Linux/Mac):**
```bash
# Basic setup
./setup-cursor-rules.sh /path/to/your/n8n-project

# Include all reference files
./setup-cursor-rules.sh -r /path/to/your/n8n-project
```

This creates `.cursor/rules/*.mdc` files that activate based on:
- File patterns (editing `*.json`, `workflows/**`)
- Semantic matching (asking about n8n, expressions, workflows)

See [CURSOR_SETUP.md](CURSOR_SETUP.md) for detailed configuration options.
...
|----------|---------|--------------|
| **Cursor** | Setup scripts | All 7 skills as MDC rules |
...
**Note:** Code skills (#6 JavaScript, #7 Python) are only in the complete bundle and Cursor setup.
...
```
dist/
├── n8n-mcp-skills-v1.0.0/                 ★ Extracted bundle (for Cursor/API)
...
├── setup-cursor-rules.ps1                 (PowerShell setup for Cursor)
├── setup-cursor-rules.sh                  (Bash setup for Cursor)
├── CURSOR_SETUP.md                        (Detailed Cursor configuration guide)
└── README.md                              (this file)
```
